...
**AI interview proctoring (Phases C + C2)** — `src/lib/proctoring/`. Proctoring evidence comes from two independent streams, and the distinction is load-bearing:
...
- **Camera vision (Phase C2)** — the interview agent worker (`agents/interview/src/vision.ts`) samples one frame every ~10s from the candidate's published video track, runs a vision model over it, and reports `{at, face_count, confidence}` to `POST /api/agent/interview/proctoring` (guarded by `AGENT_API_SECRET`, admin-client write, draft-only while the session is open). This is **server-side** evidence the candidate's machine cannot forge. It is kept strictly **out of the Realtime interviewer session** — feeding frames to the interviewer would make it react to what it sees mid-call, tipping off the candidate and turning monitoring into a live accusation. The interviewer remains audio-only.

Both streams report raw readings only; `summarizeProctoring` (`incidents.ts`, pure + versioned via `PROCTORING_REPORT_VERSION`, now `proctoring-v2`) owns all judgement — it drops sub-threshold noise, run-length-encodes vision samples into durations, and classifies what survives as `warning` / `critical`, tagging each incident with its `source` (`client` | `vision`). The report lands on `interview_sessions.proctoring`; raw vision samples live on `interview_sessions.proctoring_observations` until submit folds them in.

Vision inference is deliberately **biased toward missing incidents rather than inventing them**, because wrongly accusing a real candidate is the expensive failure: samples below `VISION_MIN_CONFIDENCE` are discarded rather than believed, a single stray frame spans zero time and can never clear a threshold, `multiple_faces` needs three consecutive sightings, and a gap over `VISION_MAX_SAMPLE_GAP_MS` breaks a run so a stalled worker can't read as a minute of absence.
