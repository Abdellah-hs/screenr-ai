...
|-------|-------|-------------|
| `priority/critical` | `#B60205` | Blocks other work or compliance risk |
| `priority/high` | `#D93F0B` | V1 ship blocker |
| `priority/medium` | `#FBCA04` | Polish, divergence cleanup |
| `priority/low` | `#0E8A16` | Nice-to-have |
| `decision` | `#5319E7` | Needs a product/eng choice before code |
| `tech-debt` | `#C2E0C6` | Refactor or migration of existing code |
| `compliance` | `#FF7619` | Legal, audit, or regulatory requirement |
| `state-machine` | `#0052CC` | Touches application status or transitions |
| `data-layer` | `#1D76DB` | Touches `src/lib/data/` or schema |
| `ui` | `#666666` | Touches React components or pages |
| `accepted-divergence` | `#D4C5F9` | Known PRD gap, temporarily accepted |
| `deferred-v1` | `#BFD4F2` | Parked until a decision resolves |
| `ai-interview` | `#C5DEF5` | Part of the real-time AI interview stack |

---

## Resolved decisions

### ~~D1. V1 scope: include the AI interview stack?~~ ✅ RESOLVED

**Decision:** Option B — V1 **includes** the AI interview MVP.  
**AI engine:** OpenAI (confirmed — `gpt-4o-mini` for scoring, OpenAI Realtime API for interview voice).  
**Consequence:** All previously deferred AI interview issues (D5–D16) are now active and must be created.  
**Recorded in:** this file, `docs/delta-prd.md`, `docs/prd-notes.md`.

---

## Resolved decisions

All decisions resolved. No open decisions remain.

---

### ~~D2. Email transport: keep Gmail or move to ESP?~~ ✅ RESOLVED

**Decision:** Option A — **Keep Gmail for V1**.  
**Consequence:** I11 builds templates on top of Gmail transport. ESP migration moves to post-V1.  
**Recorded in:** this file, `docs/delta-prd.md`.

---

### ~~D3. LinkedIn intake mechanism~~ ✅ RESOLVED

**Decision:** Option B — **Browser extension** for V1.  
**Consequence:** Spawn I25 (LinkedIn browser extension). CSV/API deferred.  
**Recorded in:** this file, `docs/delta-prd.md`.

---

### I25. feat(linkedin): browser extension for one-click candidate intake
...
### ~~D4. Rate-limit backend: Redis or stay in-memory?~~ ✅ RESOLVED

**Decision:** Option A — **In-memory for V1**. Single-server deployment. Redis deferred to post-V1 scaling.  
**Consequence:** Document the single-instance constraint in `docs/delta-prd.md` and `CLAUDE.md`.  
**Recorded in:** this file, `docs/delta-prd.md`.
...
Ship first. These are compliance risks, state-machine integrity issues, or AI interview backbone blockers.

---

### I9. fix(audit): write `ai_audit_log` row on every resume score

**Labels:** `priority/critical`, `tech-debt`, `compliance`, `data-layer`  
**Estimate:** 2–3 days
...
#### Scope
- [ ] Add `rubric_version` and `confidence` columns to `ai_audit_log` (migration)
...
#### Blocked by
- I15 (schema change: add `rubric_version` and `confidence` columns)

---

### I10. fix(audit): write `ai_audit_log` row on every screening-answer score
...
### I15. schema(audit): add `rubric_version` and `confidence` to `ai_audit_log`

**Labels:** `priority/critical`, `tech-debt`, `data-layer`  
**Estimate:** 1 day
...
### I13. refactor(state-machine): migrate legacy `candidate_stage_enum` values to canonical names

**Labels:** `priority/critical`, `tech-debt`, `state-machine`  
**Estimate:** 2–3 days
...
### I18. feat(ai-interview): build real-time interview backbone (LiveKit Agents + OpenAI Realtime plugin)

**Labels:** `priority/critical`, `enhancement`, `ai-interview`  
...
V1 ship blockers, parallelizable.

---

### I2a. feat(dedup): flag duplicate candidates for HR review instead of auto-merge

**Labels:** `priority/high`, `enhancement`, `data-layer`  
**Estimate:** 3–4 days
...
### I2b. feat(ui): HR duplicate review queue page

**Labels:** `priority/high`, `enhancement`, `ui`  
**Estimate:** 2–3 days  
**Blocked by:** I2a
...
### I11. feat(email): baseline transactional email surface for advance/reject flows
...
#### Blocked by
- None (D2 resolved: Gmail confirmed for V1)

---

### ~~I1. docs: update PRD + onboarding to reflect OpenAI as the V1 AI engine~~ ✅ FOLDED INTO TEMPLATES PR

**Not creating as a tracked issue.** This is ~30 min of find/replace. Do it as part of the PR that merges the templates branch (`chore/github-issue-templates`).

**Checklist:**
- [ ] Replace "Claude Opus 4.5" with "OpenAI" everywhere in `docs/prd.md`
- [ ] Update `docs/onboarding.md` AI section
- [ ] Add a note in `docs/prd-notes.md` explaining the engine choice and why
- [ ] Update `docs/delta-prd.md` — mark AI engine decision as resolved
- [ ] Update `docs/delta-prd.md` — mark D1 as resolved (V1 includes AI interview)
- [ ] No references to Claude Opus 4.5 remain in docs

---

### I14. schema(state-machine): add explicit failure states to `candidate_stage_enum`

**Labels:** `priority/high`, `tech-debt`, `state-machine`  
**Estimate:** 1–2 days
...
### I19. feat(scheduling): interview self-scheduling against system availability

**Labels:** `priority/high`, `enhancement`, `ai-interview`  
**Estimate:** 4–5 days  
**Blocked by:** None

#### Context
After passing screening, candidates self-serve schedule against AI interview availability. `interview_scheduling` state exists but no scheduler UI or availability model. This does not need LiveKit running — it just needs slots and a booking UI.
...
### I20. feat(recording): interview recording + transcript persistence

**Labels:** `priority/high`, `enhancement`, `ai-interview`, `compliance`  
**Estimate:** 3–4 days  
**Blocked by:** I18
...
- `src/lib/data/candidates.ts` — `saveInterviewRecording()` (new data helper)
...
### I21. feat(scoring): interview scoring (per-section + overall)
...
Polish, divergence cleanup, and AI interview supporting features.

---

### I3. feat(scoring): stamp rubric version on score writes and show mismatch badge

**Labels:** `priority/medium`, `enhancement`, `ui`  
**Estimate:** 2–3 days
...
### I4. feat(parser): add DOCX support to resume parser
...
### I6. chore(copy): rename `moderate` tier label to `Potential Match` in UI and prompts
...
### I12a. feat(review): manager review backbone — compare schema and notes data layer

**Labels:** `priority/medium`, `enhancement`, `data-layer`  
**Estimate:** 2–3 days
...
### I12b. feat(ui): manager review backbone — side-by-side compare + notes panel
...
### I16. fix(ui): mobile pass on candidate-facing pages (`/respond/[token]`)

**Labels:** `priority/medium`, `tech-debt`, `ui`  
**Estimate:** 2–3 days
...
### I22. feat(proctoring): basic interview proctoring (tab focus + presence)

**Labels:** `priority/medium`, `enhancement`, `ai-interview`  
**Estimate:** 3–4 days  
**Blocked by:** I18
...
### I23. feat(content): interview prep guide page
...
## Low priority / tech-debt issues

Nice-to-have or cleanup.

---

### I5. chore(flags): hide team-reviewers-editor behind a feature flag

**Labels:** `priority/low`, `tech-debt`, `ui`  
**Estimate:** 1 day
...
### I7. docs: document text-screening divergence as accepted temporary gap

**Labels:** `priority/low`, `accepted-divergence`  
**Estimate:** 0.5 day  
**Note:** **Moot if I24 ships in V1.** If I24 (video screening) makes it, this doc work is just updating `docs/delta-prd.md` to say the divergence is closed. If I24 slips, this documents the gap.
...
### ~~I17. chore(process): adopt issue templates + working-principles workflow~~ ✅ FOLDED INTO TEMPLATES PR

**Not creating as a tracked issue.** This is the PR description for merging `chore/github-issue-templates`.

**Checklist:**
- [ ] Merge `chore/github-issue-templates` branch
- [ ] Add `docs/working-principles.md` if it doesn't exist (or update it)
- [ ] Update `CLAUDE.md` or `docs/onboarding.md` to reference the workflow
- [ ] Announce in team chat / standup

---

### ~~I8. Confirm 3-year audit retention with legal / compliance~~ ✅ DROPPED

**Not creating as a tracked issue.** The original seed had I8 as a soft action to get written confirmation from legal. This is a calendar invite / email thread, not code work. If legal confirms the 3-year policy, document it in `docs/delta-prd.md`. If they don't, escalate to a compliance risk ticket separately.

---

### I24. feat(intake): add video/audio recording to screening questions (share LiveKit infra)

**Labels:** `priority/low`, `enhancement`, `accepted-divergence`  
**Estimate:** 3–4 days  
**Blocked by:** I18
...
## Deferred issues (do not create yet)

These stay in `deferred-v1` until D2–D4 resolve or post-V1 planning.

| ID | Title | Deferred because |
|----|-------|------------------|
| D25 | Final interview scheduling (Google Calendar) | After manager review; can be manual for V1 |
| D26 | Interview replay / highlight reel | Depends on I20 (recordings) |
| D27 | Score comparison view (advanced) | I12a/b covers basics |
| D28 | Scoring methodology documentation | UI can wait |
| D29 | Talent pool (silver-medalist) | V1.5 feature |
| D30 | Bulk actions | V1.5 feature |
| D31 | SLA enforcement jobs | Config exists; no consumer |
| D32 | Auto-archiving | Needs SLA enforcement |
| D33 | No-show handling (automated) | Depends on I19 (scheduling) |
| D34 | Reusable template library | Nice-to-have |
| D35 | Skill fingerprint | No code |
| D36 | Bias auditor | Regulatory; EU AI Act |
| D37 | Candidate experience surveys | V1.5 |
| D38 | Team fit prediction | V1.5 |
| D39 | Coaching mode public practice | V1.5 |
| D40 | Predictive analytics | V1.5 |
| D41 | Admin notifications & alerts | No surface to alert on yet |
| D42 | AI reference check | Optional stage; no code |
| D43 | Calibration & drift monitoring | Needs audit-log fix first |
| D44 | Audit log admin UI + 3-year export | Needs compliance confirmation |
| D45 | AI-personalized rejection emails | Plain templates (I11) suffice for V1 |
| D46 | Multi-language support | Post-V1 |
| D47 | Smart difficulty adaptation | Post-V1 |
| D48 | Live simulations (incident response, PR review) | Post-V1 |
...
1. Create labels
2. Create I15 (schema — blocks I9/I10)
3. Create I9, I10, I13 (critical — compliance + state machine)
4. Create I18 (critical — AI interview backbone)
5. Create I2a, I11, I14 (high)
6. Create I19, I20, I21 (high — AI interview pipeline)
7. Create I3, I4, I6, I12a, I16, I22, I23, I25 (medium)
8. Create I2b (blocked by I2a)
9. Create I12b (blocked by I12a)
10. Create I5, I7, I24 (low)

**Folded into PRs (not tracked as issues):** I1, I17  
**Dropped (not tracked):** I8  
**Total active issues: 24** (0 open decisions + 24 implementation).
**Deferred issues: 24**.
